...
import { UserDto, ProductDto } from './test-dtos';
...
describe('TranslateService', () => {
  ...
  describe('translateArray', () => {
    it('应该正确翻译对象数组', async () => {
      const users = [
        new UserDto({ id: 1, name: '张三', status: 1, genderCode: 'M' }),
        new UserDto({ id: 2, name: '李四', status: 2, genderCode: 'F' }),
        new UserDto({ id: 3, name: '王五', status: 3, genderCode: 'U' })
      ];
    ...
    it('应该处理混合类型的数据', async () => {
      const mixedData = [
        new UserDto({ id: 1, name: '张三', status: 1, genderCode: 'M' }),
        new ProductDto({ id: 1, name: 'iPhone', categoryId: 101, status: 1 })
      ];
